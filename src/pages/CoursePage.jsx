...
export default function CoursePage() {
  ...
  const { getCourseProgress, isLessonComplete, getQuizResult, getModuleProgress, isModuleUnlocked, isQuizUnlocked } = useProgress();
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="space-y-4">
        {course.modules.map((mod, i) => {
          ...
          const unlocked = isModuleUnlocked(course, mod.id);
          const quizUnlocked = mod.quiz && isQuizUnlocked(course, mod.id);

          return (
            <div key={mod.id} className={`card overflow-hidden ${unlocked ? '' : 'opacity-80'}`}>
              <div className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3">
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                      ...
                      {!unlocked ? <Lock className="w-4 h-4" /> : allLessonsDone ? <CheckCircle className="w-4 h-4" /> : i + 1}
                    </span>
                    <div>
                      <h3 className={`font-semibold ${unlocked ? 'text-gray-900' : 'text-gray-500'}`}>{mod.title}</h3>
                      {mod.time && (
                ...
                {!unlocked ? (
                  <div className="flex items-center gap-2 text-sm text-gray-500 bg-gray-50 rounded-lg px-3 py-2.5">
                    ...
                    Complete the previous session (all lessons and its quiz) to unlock this one.
                  </div>
                ) : (
                  <>
                    <ProgressBar value={modProg.completed} max={modProg.total} size="sm" showLabel={false} color={allLessonsDone ? 'green' : accent} />

                    <div className="mt-4 space-y-1">
                      {mod.lessons.map((lesson) => {
                        const done = isLessonComplete(lesson.id);
                        return (
                          <Link key={lesson.id} to={`/course/${courseId}/module/${mod.id}/lesson/${lesson.id}`}
                            ...
                            <span className={`text-sm flex-1 ${done ? 'text-gray-500' : 'text-gray-700'}`}>{lesson.title}</span>
                      ...
                      {mod.quiz && (quizUnlocked ? (
                        <Link to={`/course/${courseId}/module/${mod.id}/quiz`}
                          ...
                          <FileQuestion className={`w-4 h-4 flex-shrink-0 ${quizResult?.passed ? 'text-green-500' : 'text-amber-500'}`} />
                          <span className="text-sm flex-1 font-medium text-gray-700">{mod.quiz.title}</span>
                          {quizResult ? (
                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${quizResult.passed ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
                        ...
                        <div className="flex items-center gap-3 py-2 px-3 rounded-lg mt-2 border-t border-gray-100 pt-3 cursor-not-allowed">
                          <Lock className="w-4 h-4 flex-shrink-0 text-gray-300" />
                          <span className="text-sm flex-1 font-medium text-gray-400">{mod.quiz.title}</span>
                          <span className="text-xs text-gray-400">Finish all lessons to unlock</span>
                        </div>
                      ))}
                    </div>
                  </>
