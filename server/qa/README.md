...
# 2. Run the suites (from server/, so better-sqlite3 resolves)
node qa/qa-suite.mjs                       # 59 checks: workflows + adversarial
DB_PATH=/tmp/qa-test.db node qa/qa-phase2.mjs   # 18 checks: money math, tier gate/promotion, SLA fixtures
```
